...
const BuisnessAccount = (props) => {
  ...
  const UpdateInfosBuisness = () => {
    ...
    console.log(userData);
     const localtoken = localStorage.getItem("myJWT");
    const decoded = jwt.verify(localtoken, "secret");
      axios.post("http://localhost:8082/buisness/" + decoded.id, {
    ...
    <div>
      <div id="content_user_account">
        ...
        {/* CONTACT NAME */}
            <label>
              ...
              <input type="text" name="contact_name"
                value={userData.contact_name}  onChange={(e) => {handleChange(e); }} />
        {/* NUMBER EMPLOYES */}
        <label>
                <b>Number of employes : {userData.number_employes}</b>
              </label>
              <input type="number" name="number_employes"
                value={userData.number_employes}  onChange={(e) => {handleChange(e); }} />
        {/* ADRESS*/}
              <label>
       ...
       {/* POSTAL CODE*/}
        ...
        {/* BOUTON*/}
              <button className="button_form" onClick={e =>UpdateInfosBuisness()} id="submit_change" value="change"
              >Change info</button>
            </div>
          </div>
        )}
        ...
        <div id="candidacy_user">
          <h3>Annoucements</h3>
          <button>Create new Annoucement</button>
          <ul>
            <li>"Annonce1"</li>
            <li>"Annonce2"</li>
          </ul>
